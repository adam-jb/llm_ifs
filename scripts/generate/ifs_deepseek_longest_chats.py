...
## saving here to get embeddings
convos = []
for i in range(SAMPLE_SIZE):
     sample_convo = df['conversation'][i].str.join('\n')[0]
     convos.append({'number':i, 'conversation_text':sample_convo})

df = pl.DataFrame(convos)
print(df.head())
df.write_csv('lmsys_data/lmsys_top_1000.csv')
...
#         ## truncate if long
#         if len(sample_convo) > 25_000:
#             sample_convo = sample_convo[:25_000]
...
#             with open(f"outputs/lmsys_deepseek_{model_type}_longest_chats_rawest_numbered.txt", 'w') as output:
